...
@LargeTest
class ViewAlbumDetalleTestHU02 {
    @get:Rule
    ...
    fun DetalleAlbum() {
        ...
        val nombreAlbum2 =
            Espresso.onView(
                Matchers.allOf(
                    withId(R.id.AlbumNameTest), withText("A Night at the Opera"),
                    isDisplayed()
                )
            )
        nombreAlbum2.check(ViewAssertions.matches(isDisplayed()))

        val AlbumCover = Espresso.onView(
            first(
                withId(R.id.AlbumCover)
            )
        )
        AlbumCover.check(ViewAssertions.matches(isDisplayed()))
